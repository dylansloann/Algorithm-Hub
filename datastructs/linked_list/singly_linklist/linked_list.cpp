...
// default constructor
Node::Node() : value(0), link(nullptr) {}
...
List::List(const List& other) {
	...
	if (other.head == nullptr) { head = nullptr; tail = nullptr; _size = 0; }
	...
	// creates new nodes
		...
		while (otherCurrent->link != nullptr) {
			Node* node = new Node(otherCurrent->link->value);
			current->link = node;
...
List& List::operator=(const List& rhs) {
	...
	// if rhs list is empty
	if (rhs.head == nullptr) { head = nullptr; tail = nullptr; _size = 0; }
	...
	// creates new nodes
		...
		while (rhsCurrent->link != nullptr) {
			Node* node = new Node(rhsCurrent->link->value);
			current->link = node;
			current = current->link;
...
int List::front() { return head->value; }

// obtains value of last node
int List::back() { return tail->value; }
